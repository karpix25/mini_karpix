...
# <-- НАЧАЛО ИЗМЕНЕНИЙ 2: ПЕРЕРАБОТАННАЯ ФУНКЦИЯ АУТЕНТИФИКАЦИИ -->
async def get_current_user(x_init_data: str = Header(None), db=Depends(get_db_connection)):
    ...
    user_data_from_tg = validate_init_data(x_init_data, BOT_TOKEN)
    if not user_data_from_tg: 
        raise HTTPException(status_code=401, detail="Invalid InitData")
    
    user_id = user_data_from_tg.get("id")

    # Новая логика проверки статуса пользователя в БД
    cur = db.cursor()
    cur.execute("SELECT is_active FROM channel_subscribers WHERE telegram_id = %s", (user_id,))
    db_user = cur.fetchone()
    cur.close()

    # Если пользователя нет в базе ИЛИ он неактивен - доступ запрещен
...
def load_course_metadata(course_path: str) -> dict:
    ...
    return {
        ...
        "rank_required": 1,
        "sections": [{"id": "main", "title": "Основной раздел", "lessons": []}]
    }
...
def scan_course_lessons(course_path: str, sections: list) -> list:
    ...
    for section in sections:
        ...
        updated_sections.append({"id": section["id"], "title": section["title"], "lessons": lessons})
    return updated_sections

# --- НОВЫЕ ЭНДПОИНТЫ ДЛЯ КУРСОВ ---

# <-- НАЧАЛО ИЗМЕНЕНИЙ 3: ПЕРЕРАБОТАННЫЙ ЭНДПОИНТ СПИСКА КУРСОВ -->
...
# <-- НАЧАЛО ИЗМЕНЕНИЙ 4: ПЕРЕРАБОТАННЫЙ ЭНДПОИНТ ДЕТАЛЕЙ КУРСА -->
...
async def get_course_detail(course_id: str, user: dict = Depends(get_current_user), db=Depends(get_db_connection)):
    ...
    db_user = cur.fetchone()
    points = (db_user['message_count'] * 2) if db_user else 0
    if metadata.get("rank_required", 1) > get_rank_level(points):
...
async def get_lesson_content(course_id: str, lesson_id: str, user: dict = Depends(get_current_user), db=Depends(get_db_connection)):
    ...
    lesson_file, section_id, title = None, None, lesson_id
...
async def mark_lesson_as_complete(
    ...
    try:
        ...
        # В реальной жизни здесь лучше логировать ошибку
...
async def get_leaderboard_by_period(period: Literal['7d', '30d', 'all'] = '7d', user: dict = Depends(get_current_user), db=Depends(get_db_connection)):
    ...
    query = f"""
        WITH user_scores AS (
            ...
            SELECT s.user_id, s.total_score, cs.first_name, cs.last_name, cs.username, cs.photo_url, RANK() OVER (ORDER BY s.total_score DESC, s.user_id) as rank
            FROM user_scores s JOIN channel_subscribers cs ON cs.telegram_id = s.user_id WHERE cs.is_active = TRUE
        ) SELECT * FROM ranked_users;
    """
    if period == 'all':
        query = """
            SELECT telegram_id as user_id, first_name, last_name, username, photo_url, (message_count * 2) as total_score,
                   RANK() OVER (ORDER BY message_count DESC, telegram_id) as rank
            FROM channel_subscribers WHERE is_active = TRUE AND message_count > 0;
        """
    cur = db.cursor()
    cur.execute(query)
    ...
    current_user_data = next((CurrentUserRankInfo(**u) for u in all_users if u['user_id'] == current_user_id), None)
...
async def get_me(user: dict = Depends(get_current_user), db=Depends(get_db_connection)):
    ...
    points_to_next_rank, progress_percentage = None, 100
    if next_rank_info:
    ...
    first_name, last_name, username = (db_user['first_name'], db_user['last_name'], db_user['username']) if db_user else (user.get("first_name"), user.get("last_name"), user.get("username"))
    return UserData(id=user_id, first_name=first_name, last_name=last_name, username=username, points=points, rank=current_rank_info.name, next_rank_name=next_rank_info.name if next_rank_info else "Max", points_to_next_rank=points_to_next_rank, progress_percentage=progress_percentage)
...
async def get_all_ranks(user: dict = Depends(get_current_user), db=Depends(get_db_connection)):
    ...
    for i, rank in enumerate(RANKS): 
        ranks_list.append(RankInfo(level=i + 1, name=rank.name, min_points=rank.min_points, is_unlocked=(points >= rank.min_points)))
    return ranks_list
